function R = ValueCells_dailySV(subject1,subject2,sv,range)
...
% subject1
for iJ = 1:length(sv.subject1.low)
    svlow{iJ,1} = interp1(probs,probs,sv.subject1.low{iJ},'nearest');
    svhigh{iJ,1} = interp1(probs,probs,sv.subject1.high{iJ},'nearest');
    if svlow{iJ,1} == 0 || isnan(svlow{iJ,1})
        svlow{iJ,1} = 0.05;
    end
    if svhigh{iJ,1} == 0 || isnan(svhigh{iJ,1})
        svhigh{iJ,1} = 0.05;
    end
    x = find(svlow{iJ,1} == probs);
    eqlow.Ep1{iJ,1} = separated_trials.sub1.low.Ep1{x}.cell{iJ,1};
    eqlow.Ep2{iJ,1} = separated_trials.sub1.low.Ep2{x}.cell{iJ,1};
    clear x;
    x = find(svhigh{iJ,1} == probs);
    eqhigh.Ep1{iJ,1} = separated_trials.sub1.high.Ep1{x}.cell{iJ,1};
    eqhigh.Ep2{iJ,1} = separated_trials.sub1.high.Ep2{x}.cell{iJ,1};
    clear x svlow svhigh;
end
% subject2
for iJ = 1:length(sv.subject2.low)
    svlow{iJ,1} = interp1(probs,probs,sv.subject2.low{iJ,1},'nearest');
    svhigh{iJ,1} = interp1(probs,probs,sv.subject2.high{iJ,1},'nearest');
    if svlow{iJ,1} == 0 || isnan(svlow{iJ,1})
        svlow{iJ,1} = 0.05;
    end
    if svhigh{iJ,1} == 0 || isnan(svhigh{iJ,1})
    ...
    x = find(svlow{iJ,1} == probs);
    eqlow.Ep1 = cat(1,eqlow.Ep1,separated_trials.sub2.low.Ep1{x}.cell{iJ,1});
    eqlow.Ep2 = cat(1,eqlow.Ep2,separated_trials.sub2.low.Ep2{x}.cell{iJ,1});
    clear x;
    x = find(svhigh{iJ,1} == probs);
    eqhigh.Ep1 = cat(1,eqhigh.Ep1,separated_trials.sub2.high.Ep1{x}.cell{iJ,1});
    eqhigh.Ep2 = cat(1,eqhigh.Ep2,separated_trials.sub2.high.Ep2{x}.cell{iJ,1});
    clear x svlow svhigh;
end

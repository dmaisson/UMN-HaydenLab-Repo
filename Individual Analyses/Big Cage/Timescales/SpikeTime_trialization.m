function psth = SpikeTime_trialization(in,lfp)
...
window_2seconds = 60; %30 frames is 1 second, so 60 frames (i.e. 60 time bins) is 2 seconds of data

n = size(spike_idx,2); %how many trials there will be, based on how many spikes there were

psth = zeros(n,window_2seconds); %initialize an empty variable

if lfp == 0 %if we're dealing with spikes
    for iA = 1:n %iterate through the trials
        if spike_idx(iA)+(window_2seconds-1) < s %if there are enough data points left in the series to fit into a trial
            psth(iA,:) = spikeSeries(1,spike_idx(iA):spike_idx(iA)+(window_2seconds-1)); %take the spike as the first time point, and add 2 second of the signal to the rest of the trial
    ...
    for iA = 1:n
        if spike_idx(iA)+(window_2seconds-1) < s
            psth(iA,:) = lfpSeries(1,spike_idx(iA):spike_idx(iA)+(window_2seconds-1));
        else

function [R] = Wrapper_MultiReg_EVSep_Zvars(data,epoch1,ms)
...
for iL = 1:size(data,1)
    ...
    x(:,1) = data{iL}.zvars(:,1); %Prob 1
    x(:,2) = data{iL}.zvars(:,2); %Size 1
    x(:,3) = data{iL}.zvars(:,4); %Prob 2
    x(:,4) = data{iL}.zvars(:,5); %Size 2
    x(:,5) = data{iL}.zvars(:,7);
    x(:,6) = data{iL}.zvars(:,9);
    [z,dev,stats] = glmfit(x,y);

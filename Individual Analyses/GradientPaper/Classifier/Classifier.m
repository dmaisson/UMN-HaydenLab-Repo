function [accuracy1, accuracy2, accuracy3, accuracy4, accuracy5] = Classifier(input,token,label)
% A function for using a Linear Classifier (binary-class SVM) to decode
% binary variable class from mean neural data. This is function is built to
% work with another that
...
start{iJ}.vars = input{iJ}.vars;
    if token == 0
        ...
        start{iJ}.E4FR = input{iJ}.psth(:,265:289);
    elseif token == 1
        start{iJ}.E1FR = input{iJ}.psth(:,155:179);
        start{iJ}.E2FR = input{iJ}.psth(:,193:217);
        start{iJ}.E3FR = input{iJ}.psth(:,225:249);
        start{iJ}.E4FR = input{iJ}.psth(:,240:264);
    end
end
start = start';
...
for iJ = 1:length(start)
    ...
    elseif label == 10 %unchosen value
        for iK = 1:length(start{iJ}.vars)
            if start{iJ}.vars(iK,9) == 1
                EVunchoice{iJ}(iK,1) = start{iJ}.vars(iK,6);
            elseif start{iJ}.vars(iK,9) == 0
                EVunchoice{iJ}(iK,1) = start{iJ}.vars(iK,3);
            end
        end
        meanEVunchoice{iJ,1} = nanmean(EVunchoice{iJ}(:,1),1);
        for iK = 1:length(start{iJ}.vars)
            if start{iJ}.vars(iK,9) == 1
                if start{iJ}.vars(iK,6) > meanEVunchoice{iJ}
                    vars{iJ}(iK,1) = 1; %EVchoice (larger/smaller than mean)
                elseif start{iJ}.vars(iK,6) <= meanEVunchoice{iJ}
                    vars{iJ}(iK,1) = 0; %EVchoice (larger/smaller than mean)
                end
            elseif start{iJ}.vars(iK,9) == 0
                if start{iJ}.vars(iK,3) > meanEVunchoice{iJ}
                    vars{iJ}(iK,1) = 1; %EVchoice (larger/smaller than mean)
                elseif start{iJ}.vars(iK,3) <= meanEVunchoice{iJ}
                    vars{iJ}(iK,1) = 0; %EVchoice (larger/smaller than mean)
                end
            end
        end
    elseif label == 11 %exp reward on previous trial
        for iK = 1:length(start{iJ}.vars)
            if iK == 1
                if start{iJ}.vars(iK,10) == 0
                    vars{iJ}(iK,1) = 0;
                elseif start{iJ}.vars(iK,10) > 0
                    vars{iJ}(iK,1) = 1;
                end
            else
                if start{iJ}.vars(iK-1,10) == 0
                    vars{iJ}(iK,1) = 0;
                elseif start{iJ}.vars(iK-1,10) > 0
                    vars{iJ}(iK,1) = 1;
                end
            end
        end

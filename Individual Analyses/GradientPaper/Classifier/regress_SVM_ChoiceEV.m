function accuracy = regress_SVM_ChoiceEV(in,token)
        ...
        for iK = 1:length(in{iJ}.vars)
            for iL = 1:bins
                if in{iJ}.vars(iK,9) == 1
                    ...
                    if in{iJ}.vars(iK,3) >= EVbin(iL,1) && in{iJ}.vars(iK,3) <= EVbin(iL,end)
                        data{iJ,1}.EVCbin{iL,1}.vars(iK,:) = in{iJ}.vars(iK,:);
                        data{iJ,1}.EVCbin{iL,1}.psth(iK,:) = in{iJ}.psth(iK,250:274);
                    else
                        data{iJ,1}.EVCbin{iL,1}.vars(iK,1:size(in{iJ}.vars,2)) = NaN;
                        data{iJ,1}.EVCbin{iL,1}.psth(iK,1:25) = NaN;
                    end
                    if in{iJ}.vars(iK,3) >= EVbin(iL,1) && in{iJ}.vars(iK,3) <= EVbin(iL,end)
                        data{iJ,1}.EVCbin{iL,1}.vars(iK,:) = in{iJ}.vars(iK,:);
                        data{iJ,1}.EVCbin{iL,1}.psth(iK,:) = in{iJ}.psth(iK,250:274);

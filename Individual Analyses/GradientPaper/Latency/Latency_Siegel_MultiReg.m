function [L] = Latency_Siegel_MultiReg(data,Results,start,duration)
...
for iJ = 1:size(window,2)
    R.latency{iJ} = OmegaSq_Sliding(data,Results,window(iJ),4);
end
for iJ = 1:size(window,2)
...
L.index.E1EV1 = find(L.latency.E1(1,:) == L.max.E1EV1);
L.max.E1Size1 = max(L.latency.E1(2,:));
L.index.E1Size1 = find(L.latency.E1(2,:) == L.max.E1Size1);
L.max.E1Prob1 = max(L.latency.E1(3,:));
L.index.E1Prob1 = find(L.latency.E1(3,:) == L.max.E1Prob1);
L.max.E1Side1 = max(L.latency.E1(4,:));
L.index.E1Side1 = find(L.latency.E1(4,:) == L.max.E1Side1);
...
% for iJ = 1:size(window,2)
%     R.latency{iJ} = OmegaSq_Sliding(data,Results,window(iJ),4,2);
% end
% for iJ = 1:size(window,2)
...
% for iJ = 1:size(window,2)
%     R.latency{iJ} = OmegaSq_Sliding(data,Results,window(iJ),4,3);
% end
% for iJ = 1:size(window,2)

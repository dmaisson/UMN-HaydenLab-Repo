function out = subset_deltas_Ep2(in,probs)
% first we imagine those neurons constitute a set P, of size S 
% (where S = number of neurons that encode value abstractly)

safe = in.safe.Ep2; %take the data from safe trials
n = length(safe); %find how many cells there are
S_range = (1:100)/100; %set the range for the size of S
eqlow = in.eqlow.Ep2; %take the data for cells from that prob bin
eqhigh = in.eqhigh.Ep2; %take the data for cells from that prob bin
...
for iK = 1:n %for the number of cells in the total set
    ...
    for iL = 1:1000
        ...
        pseudolow = in.low.Ep2{x}.cell; %take the data from that bin
        x = randi(length(probs)); %repeat
        pseudohigh = in.high.Ep2{x}.cell; %repeat

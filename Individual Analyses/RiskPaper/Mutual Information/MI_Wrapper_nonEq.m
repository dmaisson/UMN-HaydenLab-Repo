function R = MI_Wrapper_nonEq(subject1,subject2,sv,range)
...
%% Mutual Information
for iJ = 1:100
    [MuIn{iJ,1},sigrate{iJ,1}] = Mutual_Information_TimeSeries_nonEq(separated_trials);
    sigrate_samples.Ep1.safe_low(iJ,:) = sigrate{iJ}.Ep1.safe_low;
    sigrate_samples.Ep1.safe_high(iJ,:) = sigrate{iJ}.Ep1.safe_high;
    sigrate_samples.Ep2.safe_low(iJ,:) = sigrate{iJ}.Ep2.safe_low;
    sigrate_samples.Ep2.safe_high(iJ,:) = sigrate{iJ}.Ep2.safe_high;
end
sigrate_samples.Ep1.safe_low = sort(sigrate_samples.Ep1.safe_low);
sigrate_samples.Ep1.safe_high = sort(sigrate_samples.Ep1.safe_high);
sigrate_samples.Ep2.safe_low = sort(sigrate_samples.Ep2.safe_low);
sigrate_samples.Ep2.safe_high = sort(sigrate_samples.Ep2.safe_high);

%% Collect
R.MuIn = MuIn;
R.sigrate = sigrate;
R.sigrate_samples = sigrate_samples;

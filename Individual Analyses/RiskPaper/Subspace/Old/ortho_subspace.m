function R = ortho_subspace(in,sv)
%% define variables
probs = 0:0.05:1;
...
for iJ = 1:n %for each cell
    for iK = 1:size(safe.Ep1{iJ}.vars,1) %for each trial for that cell
        left.Ep1{iJ,1}(iK,1:50) = NaN; %create NaN array with trial rows and 50 time bins
        right.Ep1{iJ,1}(iK,1:50) = NaN; %create another
    ...
    for iK = 1:size(safe.Ep2{iJ}.vars,1)
        left.Ep2{iJ,1}(iK,1:50) = NaN;
        right.Ep2{iJ,1}(iK,1:50) = NaN;
...
%% Correlations - Maybe don't need this
% % cluster first?
% for iJ = 1:size(safemat,2)
%     for iK = 1:size(safemat,2)
%         safecorr(iJ,iK) = corr(safemat(:,iJ),safemat(:,iK));
%     end
% end
% for iJ = 1:size(lowmat,2)
%     for iK = 1:size(lowmat,2)
%         lowcorr(iJ,iK) = corr(lowmat(:,iJ),lowmat(:,iK));
%     end
% end
% for iJ = 1:size(highmat,2)
%     for iK = 1:size(highmat,2)
%         highcorr(iJ,iK) = corr(highmat(:,iJ),highmat(:,iK));
%     end
% end
